...
// que celle du professeur connecté si il est professeur de spécialité
$stmt = $db->prepare(
...
$stmt = $db->prepare(
    "SELECT  ETUDIANT.ID_ETUDIANT,NOM_ETUDIANT,PRENOM_ETUDIANT,ADR_ETUDIANT,CP_ETUDIANT,VILLE_ETUDIANT,etudiant.EMAIL,TEL_ETUDIANT,COUNT(ID_DEMARCHE)  AS NB_DEM  
    FROM etudiant   
    ...
    WHERE etudiant.ID_PROF =:id_courant AND SPECIALITE.ID_PROF!=:id_courant
...
$stmt = $db->prepare(
    "SELECT  ETUDIANT.ID_ETUDIANT,NOM_ETUDIANT,PRENOM_ETUDIANT,ADR_ETUDIANT,CP_ETUDIANT,VILLE_ETUDIANT,etudiant.ID_CLASSE,etudiant.EMAIL,TEL_ETUDIANT,COUNT(ID_DEMARCHE)  AS NB_DEM  
      FROM etudiant 
          LEFT JOIN demarche ON ETUDIANT.ID_ETUDIANT=DEMARCHE.ID_ETUDIANT 
          INNER JOIN classe ON ETUDIANT.ID_CLASSE=CLASSE.ID_CLASSE 
          INNER JOIN specialite ON SPECIALITE.ID_PROF=ETUDIANT.ID_SPECIALITE 
...
//JA ?
//Consultation par les professeurs référent de toute la classe
$stmt = $db->prepare(
    "SELECT  ETUDIANT.ID_ETUDIANT,NOM_ETUDIANT,PRENOM_ETUDIANT,ADR_ETUDIANT,CP_ETUDIANT,VILLE_ETUDIANT,etudiant.ID_CLASSE,etudiant.EMAIL,TEL_ETUDIANT,COUNT(ID_DEMARCHE)  AS NB_DEM  
    FROM etudiant 
      LEFT JOIN demarche ON ETUDIANT.ID_ETUDIANT=DEMARCHE.ID_ETUDIANT 
      INNER JOIN classe ON ETUDIANT.ID_CLASSE=CLASSE.ID_CLASSE    
   WHERE LIBELLE_CLASSE=:id_courant 
   GROUP BY ETUDIANT.ID_ETUDIANT  
ORDER BY `etudiant`.`ID_CLASSE` ASC"
);
$stmt->bindValue(':id_courant', $id_courant, PDO::PARAM_INT);
$stmt->execute(); 
$classeProfRef = $stmt->fetchAll(PDO::FETCH_BOTH);
$countDemarcheProfspe = count($etudiantsProfSpeDemarche);
